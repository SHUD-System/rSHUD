...
getArea <- function(pm = read_mesh()){
  ...
  if (inherits(spm, "sf")) {
    ia <- as.numeric(sf::st_area(spm))
    return(ia)
  }
  
  if (inherits(spm, "SpatVector")) {
    ia <- as.numeric(terra::expanse(spm, unit = "m", transform = TRUE, by_geom = TRUE))
    return(ia)
  }
  
  if (inherits(spm, c("Spatial", "SpatialPolygons", "SpatialPolygonsDataFrame"))) {
    spm <- terra::vect(spm)
    ia <- as.numeric(terra::expanse(spm, unit = "m", transform = TRUE, by_geom = TRUE))
    return(ia)
  }
  
  stop("Unsupported mesh geometry type in getArea()", call. = FALSE)
}
...
getRiverNodes <- function(spr = read_river_sp()){
  ...
  ft = get_from_to_nodes(spr, coords = ft.pts)
